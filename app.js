...
function processToken(token) {
  ...
  if (/^[0-9A-F]{4,6}$/i.test(hexClean) && !(token.length === 2 && /^[A-Z]{2}$/.test(token) && !token.startsWith('#'))) {
    ...
    // Append Variation Selector-15 (\uFE0E) to non-flag codepoints to force monochrome text mode
  ...
  return { text: token + '\uFE0E', isFlag: false };
}
...
function updateFlag() {
  ...
  for (const token of tokens) {
    ...
    // Apply scaling and flips first, then rotations, and translate last
    const transforms = [];
