...
function processToken(token) {
  if (!token) return { text: '', isFlag: false, font: 'Catrinity' };
  ...
  if (/^[a-z0-9]+$/.test(token)) {
    ...
    sequence += '\u{E007F}';
    return { text: sequence, isFlag: true, font: 'CatrinityFlags' };
  }
  ...
  if (token.length === 2 && /^[A-Z]{2}$/.test(token)) {
    ...
    return { text: String.fromCodePoint(char1, char2), isFlag: true, font: 'CatrinityFlags' };
  }
  ...
  if (/^[0-9A-F]{4,6}$/i.test(hexClean)) {
    ...
    return { text: textChar, isFlag: isFlagRange, font: isFlagRange ? 'CatrinityFlags' : 'Catrinity' };
  }

  // 4. Fallback: Plain text
  return { text: token, isFlag: false, font: 'Catrinity' };
}
...
/**
 * Native Canvas Crisp Exporter
 * Direct High-DPI rasterization + Font Routing + Nearest Neighbor + Alpha binarization
 */
function captureCrispCanvas(targetWidth = 280, targetHeight = 280) {
  ...
  for (const token of tokens) {
    ...
    // ROUTE FONT SPECIFICALLY TO CANVAS CONTEXT
    const fontFamily = font === 'CatrinityFlags' ? 'CatrinityFlags' : 'Catrinity';
    highCtx.font = `${baseFontSize}px "${fontFamily}", sans-serif`;
  ...
  // Downsample to crisp canvas using Nearest Neighbor
  ...
  // Alpha Channel Binarization Threshold (Removes blurred border pixels)
