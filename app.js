// --- DYNAMIC FONT LOADING CHECK ---
if (document.fonts) {
  document.fonts.ready.then(() => updateFlag());
}
...
function updateFlag() {
  ...
  for (const token of tokens) {
    ...
    const { text, isFlag, font } = processToken(token);
    if (!text) continue;
 ...
 * Direct High-DPI rasterization + Nearest Neighbor + Alpha binarization
 */
