...
document.body.innerHTML = `
  ...
  <div class="container">
    ...
    <div class="hint">
      ...
      • <b>Modifiers (Pre-token):</b> <code>!fv RU !layer !s40 !dy-25 !c(#FFD700) #2605</code>
    </div>
    <input type="text" id="code" placeholder="e.g., !fv RU !layer !s40 !dy-25 !c(#FFD700) #2605 !layer !s40 !dx-20 !dy10 !c(#FFD700) #2605 !layer !s40 !dx20 !dy10 !c(#FFD700) #2605" value="!fv RU !layer !s40 !dy-25 !c(#FFD700) #2605 !layer !s40 !dx-20 !dy10 !c(#FFD700) #2605 !layer !s40 !dx20 !dy10 !c(#FFD700) #2605" autofocus>
...
function processToken(token) {
  ...
  // 1. Lowercase ISO 3166-2 Subdivision Tag Sequences
  ...
  // 2. Hex Codepoints (PUA vs Standard Characters)
  if (/^[0-9A-F]{4,6}$/i.test(hexClean) && !(token.length === 2 && /^[A-Z]{2}$/.test(token) && !token.startsWith('#'))) {
    const codePoint = parseInt(hexClean, 16);

    // Strict Flag Ranges: PUA FDE00–FDEFF, Regional Indicators 1F1E6–1F1FF, Tag Sequence 1F3F4
    const isFlagRange = (codePoint >= 0xFDE00 && codePoint <= 0xFDEFF) ||
                        (codePoint >= 0x1F1E6 && codePoint <= 0x1F1FF) ||
                        codePoint === 0x1F3F4;

    return { text: String.fromCodePoint(codePoint), isFlag: isFlagRange };
  }

  // 3. Uppercase 2-letter ISO 3166-1 Country Flags
  ...
  // 4. Fallback: Standard text / characters
...
async function captureDisplayCanvas() {
  ...
  throw new Error('html2canvas library is loading. Please try again.');
}
